...
let MailService = class MailService {
    ...
    getEmailTemplate(title, message, code, expiryMinutes) {
        return `
      <!DOCTYPE html>
    ...
    async sendVerificationEmail(email, code) {
        const html = this.getEmailTemplate('تأكيد البريد الإلكتروني', 'شكراً لتسجيلك معنا! للمتابعة، يرجى استخدام كود التحقق التالي لتفعيل حسابك:', code, 10);
        try {
            ...
            throw new common_1.InternalServerErrorException('فشل إرسال البريد الإلكتروني');
        }
    }
    async sendResetPasswordEmail(email, code) {
        const html = this.getEmailTemplate('إعادة تعيين كلمة المرور', 'تلقينا طلباً لإعادة تعيين كلمة المرور الخاصة بك. استخدم الكود التالي لإتمام العملية:', code, 10);
        try {
            await this.transporter.sendMail({
                from: `"مشروع التخرج" <${process.env.EMAIL_USER}>`,
                to: email,
